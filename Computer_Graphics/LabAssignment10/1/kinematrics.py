...
def render(t):
    ...
    R2 = np.identity(4)
    R3 = np.identity(4)
    euler2 = np.array([1.,0.,0.])*t
    R2=XYZEulerToRotMat(euler2)
    euler3 = np.array([0.,0.,1.])*t
    R3=XYZEulerToRotMat(euler3)
    T1 = np.identity(4)
    T1[0][3] = 1.
    ...
    curr_pos = J2 @ np.array([1., 0., 0., 1.])

    diff = curr_pos - pre_pos
    velocity = curr_pos + diff * 10
    pre_pos = curr_pos
    ...
    # Draw velocity of End point
    glBegin(GL_LINES)
    glColor3ub(255, 0, 0)
    glVertex3fv(curr_pos[:3])
    glVertex3fv(velocity[:3])
    glEnd()
